        ...
        for(int y = 0; y < m_camera.Height; y += CELL_SIZE_Y)
        {
            for(int cx = 0; cx < CELL_SIZE_X; cx++)
            {
                for(int cy = 0; cy < CELL_SIZE_Y; cy++)
                {
                    ...
                    Colour rayColour;
                    if(traceRay(m_camera.GetPosition(), direction, worldObjects, &m_camera, 5, rayColour))
                    {
...
bool Raytracer::traceRay(glm::vec3 origin, glm::vec3 direction, std::vector<WorldObject*> worldObjects, WorldObject* parentObject, int maxRecursion, Colour& rayColour)
{
